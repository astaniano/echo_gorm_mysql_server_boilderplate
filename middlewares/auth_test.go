...
//	}
//
//	token, err := jwtWrapper.GenerateToken(controllers_user.Email)
//	assert.NoError(t, err)
//
//	err = controllers_user.HashPassword(controllers_user.Password)
//	assert.NoError(t, err)
//
//	result := database.GlobalDB.Create(&controllers_user)
...
//
//	database.GlobalDB.Unscoped().Where("email = ?", controllers_user.Email).Delete(&models.User{})
//}